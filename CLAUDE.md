...
Runs as the `node` user (non-root) provided by the official Node.js image. System-wide tools are installed as root, user-specific tools (Oh My Zsh, plugins, uv, Claude Code) are installed after switching to the node user.
...
- User bins (`~/.cargo/bin`, `~/.claude/bin`) in Dockerfile ENV
